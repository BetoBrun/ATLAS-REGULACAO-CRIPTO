const I18N = {
  en: {
    ...
    scoreMin: 'Minimum score', topic: 'Topic', allTopics: 'All topics', exchanges: 'Exchanges', stablecoins: 'Stablecoins', tokenization: 'Tokenization', cbdc: 'CBDC', defi: 'DeFi',
  ...
  pt: {
    ...
    scoreMin: 'Pontuação mínima', topic: 'Tema', allTopics: 'Todos os temas', exchanges: 'Exchanges', stablecoins: 'Stablecoins', tokenization: 'Tokenização', cbdc: 'CBDC', defi: 'DeFi',
...
const DEFAULT_PRESETS = [
  ...
  { slug: 'defi', label_en: 'DeFi', label_pt: 'DeFi', predicateKey: 'defi' }
];
...
function presetPredicate(country, preset) {
  ...
  if (preset.predicateKey === 'defi') return !!country.status?.defi && !/absent|banned/i.test(String(country.status?.defi || ''));
  return true;
}
...
async function init() {
  ...
  const [countries, metadata, metricsFile, themesFile, updates, experiencesFile, libraryFile, timelineFile, presetsFile] = await Promise.all([
    loadJson('api/countries.json'),
    loadJson('api/metadata.json').catch(()=>({})),
    loadJson('api/metrics.json').catch(()=>({})),
  ...
  state.countries = countryList; state.filtered = countryList; state.selected = countryList.find(c=>c.iso3==='BRA') || countryList[0] || null; state.metadata = { ...(metadata || {}), ...(metricsFile || {}) }; state.themes = themesFile.themes || []; state.updates = updates || []; state.experiences = experiencesFile.experiences || []; state.library = libraryFile.items || []; state.timeline = timelineFile.events || []; state.presets = presetsFile.presets || DEFAULT_PRESETS;
  populateRegions(); applyTranslations();
  console.info('Atlas loaded', { countries: state.countries.length });
}
